...
void ESPFileServer::begin() {
  ...
  //server.begin();                                                                                     // IF YOUR DIDN'T USE WEB_UI LIBRARY OR AUTOCONNECT LIBRARY, THEN YOU CAN DIRECTLY USE "server.begin", 
  Serial.println("HTTP server started");                                                                // comment server.begin to view SPIFFS file, even user didn't connect Wifi, otherwise autoconnect webserver conflict with spiffs webserver, if user server.begin before portal.begin
}                                                                                                      // Test cases, if i set 
...
void ESPFileServer::append_page_footer(){ // Saves repeating many lines of code for HTML page footers
  ...
  webpage += F("<li><a href='/_ac'>AutoConnect</a></li>");
...
void ESPFileServer::Spiffs_handleClient() {
  server.handleClient();
}
...
void ESPFileServer::HomePage() {
  ...
  webpage += F("<a href='/_ac'><button>AutoConnect</button></a>");
  SendHTML_Content();
  SendHTML_Stop(); 
}
...
void ESPFileServer::SPIFFS_dir(){ 
  if (SPIFFS_present) { 
    ...
    if (root) {
      ...
      SendHTML_Header();
      webpage += F("<h3 class='rcorners_m'>ESP Flash(Spiffs) Memory Content</h3><br>");
