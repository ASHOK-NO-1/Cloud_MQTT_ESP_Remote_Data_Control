...
void MQTT::reconnect_client() {                                                                            // Reconnect MQTT client      // PROBLEM, MQTT PORT NUMBER , BROKER ID, USERNAME AND PASSWORD, IT BECOME WRONG VALUES OR VANISHED
    //delay(1000);
    int retryCount = 0;
    const int maxRetries = 10;                                                                              // Set a limit on the number of retries
...
// BELOW SOFT AP STATIC IP woking good in PC, but in android, autoconnect portal is not good , it fluxating, not stable, 
//In android softAP is open webpage and close again, not stable, so i use default SoftAP IP 172.217.28.1
